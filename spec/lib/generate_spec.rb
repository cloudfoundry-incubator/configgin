  ...
  context 'with some file paths and an eval context' do
    ...
    it 'should preserve template permissions' do
      begin
        ...
        Generate.generate(output: file.path, data: File.read(input_filename)) do |output, input|
          Generate.render(output, input, restricted_template_filename)
        end
    ...
    it 'should know template filename' do
      ...
      output_buffer = StringIO.new
      File.open(input_filename) do |input_file|
        Generate.render(output_buffer, input_file, know_filename_template_filename)
      end
    ...
    it 'should create directories for output paths' do
      Dir.mktmpdir('configgin_mkdir_p_test') do |dir|
        ...
        Generate.generate(output: output_file, data: File.read(input_filename)) do |output, input|
          Generate.render(output, input, template_filename)
        end
